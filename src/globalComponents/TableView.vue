<template>
    <div class="table-view">
        <el-table :data="tableData" border stripe :row-key="type === 'treeTable' ? 'cat_id' : ''" style="width: 100%">
            <el-table-column type="index" :index="1" label="#" align="center" width="50" />
            <template v-for="item in tableColumns" :key="item.prop">
                ...
                <el-table-column v-else-if="item.slots === 'action'" :label="item.label" #default="scope" align="center"
                    ...
                    <el-button v-if="item.btns!.edit" type="primary" size="small" :icon="Edit"
                        @click="handleEdit(scope.row)">{{ item.btns!.editLabel }}</el-button>
                    <el-button v-if="item.btns!.del" type="danger" size="small" :icon="Delete"
                        @click="handleDelete(scope.row)">{{ item.btns!.delLabel }}</el-button>
                ...
                <el-table-column v-else-if="item.slots === 'tag'" :label="item.label" #default="scope" align="center">
                    <template v-if="item.prop === 'level' || item.prop === 'cat_level'">
                    ...
                    <template v-else-if="item.prop === 'pay_status'">
                        <el-tag v-if="scope.row[item.prop] === '0'" type="danger">未支付</el-tag>
                        <el-tag v-else-if="scope.row[item.prop] === '1'" type="success">已支付</el-tag>
                    </template>
                </el-table-column>
                <el-table-column v-else-if="item.slots === 'icon'" :label="item.label" #default="scope" align="center">
                    <el-tag v-if="scope.row[item.prop] === false" type="success">✓</el-tag>
                </el-table-column>
        ...
        <el-pagination v-if="total > 0" :current-page="Number(searchFormData.pagenum)"
            :page-size="searchFormData.pagesize" :page-sizes="[2, 10, 20, 30]" size="small" :background="true"
